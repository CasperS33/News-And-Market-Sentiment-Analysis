...
# --- Load Sentiment Data ---
# Load data from CSV
sentiment_df = pd.read_csv('Exam files folder/Sentiment results/vader_sentiment_results_title2.csv')
...
keywords = ['GME', 'GameStop', 'gamestop', 'gme']
filtered_sentiment = sentiment_df[sentiment_df['title'].str.contains('|'.join(keywords), case=False, na=False)]
...
correlation = combined_data['Avg_sentiment_score'].corr(combined_data['GME_rel_change_to_SP500'])
print(f"Correlation between Avg. Sentiment Score and GME Relative Change to SP500: {correlation:.4f}")

# --- Step 2: Lagged Analysis ---
print("\nStep 2: Lagged Analysis")
# Introduce a 1-day lag in sentiment scores
combined_data['Lagged_sentiment'] = combined_data['Avg_sentiment_score'].shift(-2)

# Correlation after lagging sentiment scores
lagged_correlation = combined_data['Lagged_sentiment'].corr(combined_data['GME_rel_change_to_SP500'])
print(f"Correlation with 1-day Lagged Sentiment: {lagged_correlation:.4f}")

# --- Step 3: Visual Analysis ---
print("\nStep 3: Visual Analysis")

# Plot original sentiment and GME Relative Change
fig, ax1 = plt.subplots(figsize=(12, 6))

# Plot Lagged Sentiment Score
color = 'tab:blue'
ax1.set_xlabel('Date')
ax1.set_ylabel('Lagged Sentiment Score', color=color)
ax1.plot(combined_data['Date'], combined_data['Lagged_sentiment'], label='1-Day Lagged Sentiment Score', color=color)
...
fig.suptitle('1-Day Lagged Sentiment Score and GME Relative Change to SP500', fontsize=14)
fig.tight_layout()
plt.show()
